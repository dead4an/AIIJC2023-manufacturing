...
DROP_FEATURES = ['Material', 'Cancel_Complete_Release', 'Month1', 'Month2', 'Month3', 'Delivery_Date',
                 'Change_on_Paper', 'Amount', 'Category_Manager', 'NRP', 'Provider_Delivery_option']
...
class DataPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X) -> pd.DataFrame:
        ...
        # Экстракция фич
        X_['Provider Purchaser'] = [f'{x}_{y}' for x, y in zip(
            X_['Provider'].values, X_['Purchasing Organization'].values)]
        X_['Provider Delivery option'] = [f'{x}_{y}' for x, y in zip(
            X_['Provider'].values, X_['Delivery Option'].values)]
        X_['Sum Fold'] = X_['Sum'].apply(lambda x: int(x) % 10)
        X_['ETC Difference'] = X_['Duration'] - X_['ETC Delivery']
        X_['Change Difference'] = X_['Delivery Date'] - X_['Change on Paper']
        X_['ETC Power'] = X_['ETC Difference'] ^ 2
        ...
        # Нормализация
        X_[self.scale_features] = self.robust_scaler.transform(
            X_[self.scale_features])
